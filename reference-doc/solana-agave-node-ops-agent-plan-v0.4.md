...
| Field | Value |
|---|---|
| Document | Implementation plan, **v0.4** (DRAFT — for review before any code is written) |
| Author | Peng Huang (peng.huang@acm.org) |
| Date | 2026-09-21 |
| Supersedes | v0.3 (2026-09-17), v0.2 (2026-09-17), v0.1 (2026-08-10) |
| Companion | `valops-agent-mvp-hld-v0.4.md` (refines this plan for milestone M0) |
...
**W1 needs a pinned configuration first (v0.4, Review-3).** A configuration cannot be verified until it is fixed. W1 therefore starts only after D1 (language) and D7 (SDK package version and model ID) are closed in the decision register (§13.1). W1 records the exact language runtime, SDK version and model ID it ran against. A W1 result obtained on any other combination does not count towards G1, and changing any of the three later re-opens W1 (§17.1). The Python listing below is the reference for the D1 default.
...
|---|---|---|---|
| **G0 — Define** | Before M0A | The delivery package: M0 product definition (§2.6), closed M0A-blocking decisions (§13.1), approved parameter register (§13.2), gate classes and fixture-freeze procedure (§8.1), M0A charter (§7.1), governance (§17), degraded-mode runbooks (HLD §9.1) | G0 approval by the go/no-go authority (§7.2). **No M0A code before this** |
| **0 — Verify** | Phase 0 (HLD W1), first item of M0A | Spike on the SDK version, language and model **pinned by D1/D7**. Confirm: `tools=["Skill", ...]` semantics and whether skills load with `setting_sources=[]`; `dontAsk` + role-scoped allowlists; hook deny and timeout behaviour; SDK sandbox fail-closed behaviour and the `failIfUnavailable` discrepancy (§5.2); OS sandbox backend with the SDK process; structured `output_format` | Written verification log. Every mismatch amends §5.2 before Phase 1 starts |
...
| Item | Content |
|---|---|
| Objective | Demonstrate the four containment properties (HLD §13) on **real** boundaries, with no diagnosis workflow attached |
| Starts | On G0 approval. Not before |
| Time box [A] | **Proposed: 6 calendar weeks** from G0, with a checkpoint review at week 3. Confirmed or changed at G0 |
| In scope | W1–W8 (HLD §12). One stub role session, used only to exercise the worker, sandbox and brokers |
...
| SDK DRI | W1 and the verification log | — |
...
|---|---|---|---|
| W1 | Phase-0 SDK verification on the pinned configuration | **D1, D7 closed**; ER-5, ER-6 | 5 |
...
| W6 | Process topology: controller, worker spawn, token minting, OS sandbox | W1, W5; ER-6, ER-10 | 10 |
| W7 | Capability brokers: episode, observer, report | W2, W3, W4 | 6 |
| W8 | M0A containment suite on real boundaries | W3–W7 | 6 |
| | **Total** | | **57** |

The **critical path** is G0 → W3 → W4 → W7 → W8, 28 engineer-days end to end. The parallel path G0 → D1/D7 → W1 → W6 → W8 is 21 days and becomes critical if W1 finds a gap that must be closed outside the SDK. With two engineers the proposed 6-week box leaves about one week of slack. A W1 finding that invalidates the SDK sandbox or permission model is the main schedule risk, and it is a kill-criterion question (§12 item 1) before it is a schedule question.
...
| ER-5 | Language runtime, SDK version and model ID pinned (D1/D7); Claude API key in controller-only secret storage | Lockfile; secret-store access list | W1 |
...
- **SDK:** the pinned SDK version making real API calls with the pinned model.
...
| Degradation | Claude API down; store down; broker down; daemon down; budget or retry budget exhausted; anchor unreachable; **anchor lag above P-19 stops new LLM steps**. Correct degradation tier; validator unaffected; human alerted; each case exercised by following its runbook (HLD §9.1) |
...
| Controls | Deterministic by construction | **Repeatable controls,** recorded with every result: pinned model ID, SDK version, language runtime, bundle id, prompt hashes, fixture-set hash, run count per fixture (P-23). A change to any control invalidates comparison with earlier runs |
...
|---|---|---|---|---|---|---|
| **D1** | Implementation language | SDK DRI | PO | G0 | **Python** (`claude-agent-sdk`) for controller, brokers and worker. The HLD reference and the X1 inspection are both Python. v0.3 moved start/end and validation work into the controller, so the TypeScript-only hook events add little (background below) | Recorded choice; lockfile |
| **D7** | Pinned SDK version and model ID(s) | SDK DRI | PO | G0 | The current `claude-agent-sdk` release on the G0 date, pinned exactly. One current Claude model ID for **all** roles, recorded exactly at G0; the candidate as of this draft is `claude-opus-5`, to be confirmed as current then. Per-role tiering waits for M0B cost data (Q-6). C1's model names are not used | Lockfile and model ID recorded in the bundle; W1 run against both |
...
Every tunable value has one row, and each row is owned. "Proposed safe initial value" is [A] and errs in the safe direction: less load on the validator, fewer model turns and less spend, earlier hand-off to a human. Values marked *set at G0* depend on facts only the owner has. The register is versioned. The thresholds in a policy bundle are generated from it, never edited by hand, and each bundle records the register version it came from.
...
**Frozen items.** These are frozen: the §2.6 product definition; the registers in §13.1 – §13.3; the M0A charter (§7.1); the gate suites (§8, HLD §10); the model-quality fixture sets (§8.1); the degraded-mode runbooks (HLD §9.1); and the pinned language runtime, SDK version and model ID (D1/D7).
...
4. Any evaluation run whose repeatable controls changed is marked **invalid for gate purposes**. A change to the D1/D7 pins re-opens W1 and the HLD §10.1 SDK-configuration suite.
...
| Secrets | API key, token-minting key, chain-head and daemon-chain signing keys, bundle signing key, mTLS keys | Until rotated | The named holders only | Never in plaintext backups. Rotated when a holder leaves the project |

**Principles [A].**

- **Least privilege** per process class (§4.2). Humans use named accounts, and their access is logged.
- **Two egress paths only.** Data leaves the environment only through Claude API calls, which carry exactly what the role's view contains, and through alerts to the D5 channel, which are size-capped and redacted. The organisation's data-handling terms for the Claude API are to be confirmed at G0 as acceptable for testnet operational data.
...
| Claude API key | Not backed up | Re-issue from the provider; controller refuses to start without it |
...
**v0.4 结论与状态：** 根据编码前评审（V2，"Review-3"），当前为**"有条件不启动"**：在 G0 交付定义包获批之前，不编写任何 M0A 代码。v0.3 的全部安全边界保持不变。
