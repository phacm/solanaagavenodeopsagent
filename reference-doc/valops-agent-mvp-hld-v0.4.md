...
| Field | Value |
|---|---|
| Document | High-Level Design (HLD), **v0.4** — DRAFT for review |
...
| Date | 2026-09-21 |
| Supersedes | v0.3 (2026-09-17), v0.1 (2026-09-17). Version numbers are kept in lockstep with the plan |
| Parent document | `solana-agave-node-ops-agent-plan-v0.4.md` (governs; this HLD refines it) |
...
Plan v0.4 describes the full system across six phases. This HLD specifies **only M0**, the smallest deployable increment that delivers value while taking no intentional risk to the validator, and that builds the complete control-plane skeleton later milestones plug into.
...
| R-3 | Judge FAIL returns to `recommending`, at most **N** times (N = P-09); on exhaustion → `handed_off` + alert | [A] |
...
|---|---|---|---|---|
| RB-1 | Claude API unavailable or slow | In-flight episode → `handed_off` with the signals gathered so far; alert. Sentinel and verifier continue | Diagnose from watchtower and the handed-off signals. Check provider status | A probe step completes. New episodes proceed normally |
...
**M0B does not start until all five pass on real boundaries, repeated over P-22 trials, and G1 is approved** (plan §7.2).
...
The G2 metrics are Q-1 … Q-8, defined in plan §2.6 with targets in plan §13.3. The table below is what the system must emit for them and for day-to-day health. Rows not mapped to a Q-metric carry no gate weight.
...
| Cost and turns per role, per episode and per day | **Q-6**; data for P-05 … P-07 |
...
| **W8** | **M0A containment test suite (§10.1), run on real boundaries** | W3–W7 | M | **Evidence for G1** |
...
3. **A process topology that fails closed.** Four process classes with disjoint credentials; the worker holds no API key, cannot write the filesystem, and reaches nothing but the Claude API and its broker sockets; making the **real** sandbox backend unavailable prevents a worker from starting.
...
Also required: environment readiness ER-1 … ER-13 complete, and a backup-restore test passed (plan §7.1, §17.3).

### G2 — M0 → M1

M1 needs **both** safety evidence **and** operator-value evidence. Safety passing alone is not enough (plan §12 item 9).
...
| D7 | Pinned SDK version and model ID | W1, W12 | Exact SDK pin and one current model ID for all roles, recorded at G0 |
...
| D11 | OS sandbox backend | W1, W6 | bubblewrap; must fail closed on the real backend at G1 |
...
| — | Parent plan: `solana-agave-node-ops-agent-plan-v0.4.md` |
...
**v0.4 状态：** 根据编码前评审（V2），结论为**"有条件不启动"**。G0 批准交付定义包之前不编写代码；G1（M0A 在真实而非模拟的边界上通过）之后才启动 M0B；G2（安全门槛**与**值班人员价值证据均达标）之后才进入 M1。关口、决策人与否决权见计划 §7.2。
...
**v0.3 设计要点（保持不变）：** 事件存储三端点、身份由传输层认证、智能体端不存在结案能力；单一审计定序器及外部锚定；四类进程、`PreToolUse` 钩子仅为纵深防御；SDK 配置修正与提示词内联回退；比较并交换、幂等键、仅 `awaiting_operator` 可转 `resolved`、`handed_off` 为终态；撤回"可用性风险为零"并限制观测资源；遥测健康事件类；不可变策略包；产出完整性校验。
